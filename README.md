...
- после чего применяем к этой строке функцию для генерации hash-строки на основе ключа шифрования, используя метод хэширования HMAC и алгоритм шифрования SHA256.<br>
...
>}
>```
- такой ответ означает, что запрос ещё не выполнился — достаточно немного подождать (рекомендуем взять интервал в 0.5с), после чего повторно запросить результат с помощью [/request/get](#post-requestget);
