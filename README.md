...
- [Webhook и реагирование на команды](#Webhook-и-реагирование-на-команды)
    ...
    - [Доступные методы для ответа с webhook](#Доступные-методы-для-ответа-с-webhook)
...
Получив сообщение на ваш webhook, у вас есть возможность ответить на команду пользователя, отправив данные в ответе запроса. Без необходимости вызова метода Userbot API.<br>
Ответ на запрос от бота Compass должен иметь формат **json-строки** требуемых параметров (пустой, если отвечать на отправленную команду не требуется).
...
>}
>```
- version - версия API ботов. Ответ на команду бота добавилась в 3й версии API;
- method - метод, который желаете выполнить в ответе;
- text - текст сообщения от бота Compass;
- type - тип сообщения для ответа - тип "text" указывает, что сообщение будет текстовым.

#### Доступные методы для ответа с webhook

#### `/message/send`

Метод для отправки сообщения от бота на команду пользователя.

Должны быть указаны следующие параметры:

| Название | Тип | Свойство | Описание |
| -------- | --- | --- | -------- |
| version | int | обязательный | Версия API ботов. |
| method | string | обязательный | Выполняемый метод в ответе на команду. |
...
Метод для отправки сообщения от бота в тред на команду пользователя.
...
Метод для установки реакции на команду пользователя.
